...
def parse_tfrecord_fn_yuv(example):
    ...
    example = tf.io.parse_single_example(example, feature_description)
    example["image1"] = tfio.experimental.color.rgb_to_ycbcr(
        tf.io.decode_jpeg(example["image1"], channels=3)
    )
    example["image2"] = tfio.experimental.color.rgb_to_ycbcr(
        tf.io.decode_jpeg(example["image2"], channels=3)
    )
    example["image3"] = tfio.experimental.color.rgb_to_ycbcr(
        tf.io.decode_jpeg(example["image3"], channels=3)
    )
    example["image4"] = tfio.experimental.color.rgb_to_ycbcr(
        tf.io.decode_jpeg(example["image4"], channels=3)
    )
    example["image5"] = tfio.experimental.color.rgb_to_ycbcr(
        tf.io.decode_jpeg(example["image5"], channels=3)
    )
    ...
    formatted_example["label"] = (
        tfio.experimental.color.rgb_to_ycbcr(
            tf.io.decode_jpeg(example["y"], channels=3)
        )
        / 255
    )
