...
class AEMonitor(keras.callbacks.Callback):
    def __init__(self, orig_prediction_frames, outdir, offset=0, yuv=False):
        ...
        for i in range(5):
            ...
            Image.fromarray((((frame + 0) / 1) * 255).astype("uint8")).save(
                os.path.join(self.outdir, "frame%04d.jpg" % (i))
            )

    def on_epoch_end(self, epoch, logs=None):
        prediction = self.model.predict(self.frames_to_predict)[0]
        Image.fromarray(
            (((prediction + 0) / 1) * 255).astype("uint8"), mode=self.mode
        ).save(
            os.path.join(self.outdir, "frame%04d.jpg" % (int(epoch) + 5 + self.offset))
        )
        Image.fromarray(
            (((prediction + 0) / 1) * 255).astype("uint8"), mode=self.mode
        ).save(os.path.join(self.outdir, "frame.jpg"))
